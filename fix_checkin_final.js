...
// 1. Fix targetDates
content = content.replace('        for (let i = 0; i < numNights; i++) {', '        const targetDates: string[] = [];\n        for (let i = 0; i < numNights; i++) {');

// 2. Add OCR Scan Button States
if (!content.includes('isScanningSlip')) {
  const target = "  const [payCredit, setPayCredit] = useState<number | ''>('');";
  const replacement = `  const [payCredit, setPayCredit] = useState<number | ''>('');\n  const [paymentTime, setPaymentTime] = useState<string>('');\n  const [isScanningSlip, setIsScanningSlip] = useState(false);`;
  content = content.replace(target, replacement);
}

// 3. Add OCR Scan Function
...
// 4. Update the UI to include the Scan Button
