...
export default function TuyaQuotaWidget() {
  ...
  const packageTotalDays = (1 + 6) * 30; // 1 month trial + 6 months extension = ~210 days
  const packageDaysUsed = Math.floor((now.getTime() - keyStartDate.getTime()) / (1000 * 3600 * 24));
  const packageDaysLeft = Math.max(0, packageTotalDays - packageDaysUsed);
  const packagePercent = Math.min(100, (packageDaysUsed / packageTotalDays) * 100);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         ...
         <div className="bg-slate-50 p-5 rounded-xl border border-slate-100">
            ...
            <div className="flex justify-between text-sm mb-1 mt-4 font-medium">
               <span className="text-slate-500">ใช้มาแล้ว: {packageDaysUsed} วัน</span>
               <span className="text-indigo-600 font-bold">เหลืออีก: {packageDaysLeft} วัน</span>
            </div>
            <div className="w-full bg-slate-200 rounded-full h-2.5 overflow-hidden">
               <div className="bg-indigo-500 h-2.5 rounded-full transition-all duration-1000" style={{ width: `${packagePercent}%` }}></div>
            </div>
            <p className="text-xs text-slate-400 mt-3 flex justify-between">
               <span>(โควตารวม 1 + 6 เดือน)</span>
