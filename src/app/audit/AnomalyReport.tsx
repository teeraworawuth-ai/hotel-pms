    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {data.map(room => {
          ...
          return (
            <div key={room.roomId} className="bg-white rounded-2xl shadow-sm border border-red-100 overflow-hidden flex flex-col h-full">
              <div className="bg-red-50 px-5 py-4 border-b border-red-100 flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xl font-black text-red-900">{room.roomNo}</span>
                    {room.location && <span className="bg-red-200/50 text-red-700 px-2 py-0.5 rounded text-[10px] font-bold">{room.location}</span>}
                  </div>
                  <p className="text-xs text-red-700 font-medium">สูญเสีย: <span className="font-bold">{totalStr}</span> (ประมาณ <span className="font-bold">{totalCost}฿</span>)</p>
                </div>
              ...
              <div className="p-4 flex-1 bg-slate-50/30">
                <div className="space-y-3">
                  {room.sessions.map((session: any) => (
                    <div key={session.id} className={`p-3 rounded-xl border ${session.isOngoing ? 'bg-rose-50 border-rose-200' : 'bg-white border-slate-100'}`}>
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-bold text-slate-700">รอบที่ {session.id}</span>
                        {session.isOngoing ? (
                          <span className="text-[10px] font-bold bg-rose-500 text-white px-2 py-0.5 rounded-full animate-pulse">กำลังเปิดอยู่</span>
                        ) : (
                          <span className="text-[10px] font-bold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">จบแล้ว</span>
                        )}
                      </div>
                      ...
                      <div className="flex justify-between items-end mb-3">
                        <div className="text-xs text-slate-500">
                          ...
                          <div className="mt-1 font-medium text-slate-700">ใช้เวลา: {session.durationMins} นาที</div>
                        </div>
                        <div className="text-right">
                          <div className="text-[10px] text-slate-400 mb-0.5">ไฟเฉลี่ย / <span className="text-blue-500 font-bold">หน่วย (kWh)</span> / <span className="text-rose-500 font-bold">สูญเสีย</span></div>
                          <div className="text-lg font-black text-slate-700">
                            {session.avgW}<span className="text-[10px] text-slate-400 ml-0.5 mr-2">W</span>
                            <span className="text-blue-600">{session.kwhStr}</span><span className="text-[10px] text-blue-400 ml-0.5 mr-2">หน่วย</span>
                            <span className="text-rose-600">{session.estimatedCost}</span><span className="text-[10px] text-rose-400 ml-0.5">฿</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full mt-2 py-2 text-xs font-bold bg-white border-2 border-slate-200 text-slate-600 rounded-lg hover:border-blue-500 hover:text-blue-600 hover:bg-blue-50 transition-colors"
                      >
                        🔍 ตรวจสอบ/ระบุสาเหตุ
                      </button>
                    </div>
