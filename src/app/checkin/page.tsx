                    ...
                    return (
                      ...
                      >
                        ...
                        {(room.status === 'occupied' || room.status === 'reserved' || room.status === 'dirty') && (
                          <div className="absolute top-0.5 left-0 bottom-[15px] sm:bottom-[18px] w-[30%] flex flex-col justify-between items-start text-[6px] sm:text-[9.5px] leading-none font-semibold opacity-90 pl-0.5 py-0.5">
                            ...
                            {room.status === 'occupied' || room.status === 'reserved' ? (
                              <div className="flex flex-row items-center gap-0 self-start -space-x-[3px] -ml-[2px]">
                                <div className="relative inline-flex items-center justify-center">
                                  ...
                                  <span className="absolute top-[1.5px] w-full text-center text-[6px] sm:text-[8px] text-slate-900 font-black drop-shadow-sm">
                                    {room.guest_count || 1}
                                  </span>
                                </div>
                                {details && (
                                  <div className="relative inline-flex items-center justify-center">
                                    ...
                                    {details.type === 'overnight' ? (
                                      <span className="absolute top-[1px] right-[1px] text-[6px] sm:text-[8px] text-slate-900 font-black drop-shadow-sm">
                                        {details.text}
                                      </span>
                                    ) : (
                                      <span className="absolute bottom-[1px] w-full text-center text-[6px] sm:text-[8px] text-slate-900 font-black drop-shadow-sm">
                                        {details.text}
                                      </span>
                                    )}
                                  </div>
                        ...
                        <div className="flex-1 w-full h-full flex flex-col items-center justify-start pt-[22px] sm:pt-6 pb-[18px]">
                          {(() => {
                            let isOverdue = false;
                            ...
                            return (
                              <div className={`absolute top-0 text-base sm:text-xl top-0 sm:top-1 font-black ${roomNoColor} transition-all leading-none flex items-center justify-center gap-0 mix-blend-multiply`}>
                        ...
                        {((room.has_upcoming && room.upcoming_days && room.upcoming_days.length > 0) || (room.incoming_today && (room.status === 'occupied' || room.status === 'dirty'))) && (
                          <div className={`absolute top-0 right-1 bottom-[18px] flex flex-col items-end ${((room.upcoming_days?.length || 0) + (room.incoming_today && (room.status === 'occupied' || room.status === 'dirty') ? 1 : 0)) >= 6 ? 'justify-between py-1 gap-0' : 'gap-[2px] pt-1'} text-slate-400 overflow-hidden pr-0.5`}>
