...
with open('src/app/components/EnergyGraph.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Add startDragRef and isFullScreen state
state_block = r"const \[localOffset, setLocalOffset\] = useState\(0\);"
new_state_block = """const [localOffset, setLocalOffset] = useState(0);
  const [isFullScreen, setIsFullScreen] = useState(false);
  const startDragRef = useRef({ x: 0, y: 0, time: 0 });"""
content = re.sub(state_block, new_state_block, content)

# 2. Add startDragRef tracking in onMouseDown and onTouchStart
mousedown_block = r"const onMouseDown = \(e: React\.MouseEvent\) => \{\n\s*if \(\!chartContainerRef\.current\) return;"
new_mousedown_block = """const onMouseDown = (e: React.MouseEvent) => {
    startDragRef.current = { x: e.clientX, y: e.clientY, time: Date.now() };
    if (!chartContainerRef.current) return;"""
content = re.sub(mousedown_block, new_mousedown_block, content)

touchstart_block = r"const onTouchStart = \(e: React\.TouchEvent\) => \{\n\s*if \(\!chartContainerRef\.current\) return;"
new_touchstart_block = """const onTouchStart = (e: React.TouchEvent) => {
    if (e.touches.length === 1) startDragRef.current = { x: e.touches[0].clientX, y: e.touches[0].clientY, time: Date.now() };
    if (!chartContainerRef.current) return;"""
content = re.sub(touchstart_block, new_touchstart_block, content)

# 3. Add handleContainerClick
click_handler = """  const handleContainerClick = (e: React.MouseEvent) => {
...
# 4. Modify CustomTooltip
tooltip_block = r"const CustomTooltip = \(\{ active, payload \}: any\) => \{[\s\S]*?return null;\n  \};"
new_tooltip_block = """const CustomTooltip = ({ active, payload }: any) => {
...
content = re.sub(tooltip_block, new_tooltip_block, content)

# 5. Fix duplicate 7 (Set logic)
set_logic = r"return ticks\.sort\(\(a, b\) => a - b\);"
new_set_logic = "return Array.from(new Set(ticks)).sort((a, b) => a - b);"
content = re.sub(set_logic, new_set_logic, content)

# 6. Replace Return Block
return_regex = r"return \(\n\s*<div className=\"mt-2 relative w-full group select-none\">[\s\S]*?\);\n\}"
new_return = """const renderChartContent = () => (
...
"""
content = re.sub(return_regex, new_return, content)

with open('src/app/components/EnergyGraph.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
